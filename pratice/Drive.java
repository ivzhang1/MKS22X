...
public class Drive{
	...
	//Quick.swap(start, 0, start.length-1);
        //System.out.println(Arrays.toString(start));
	//Quick.insertionsort(start, 0, start.length-1);
	//Quick.quicksort(start, 0, start.length-1);	
	//Quick.mergesort(start, 0, start.length-1);
	Sorties.quicksort(start);
	//Sorties.mergesort(start);

	long elapsedTime = System.currentTimeMillis() - startTime;
		
	//long startTimeI = System.currentTimeMillis();

	//Sorties.insertionsort(start, 0, start.length-1);
	//Sorties.quicksort(start);
	Sorties.mergesort(start);
